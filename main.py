...
def process_trades(trades):
    ...
    for i, trade in enumerate(trades):
        try:
            ...
            # execute_daily_tradesで調整済みのdatetimeオブジェクトがある場合はそれを使用
...
if DISCORD_BOT_TOKEN:
    ...
    async def all(ctx):
        """全体の取引パフォーマンスレポート"""
        try:
            await ctx.send('📊 全体パフォーマンスレポートを生成中...')
            report = get_performance_report(use_today_only=False)
            
            # 長いレポートは分割
            if len(report) > 2000:
                chunks = [report[i:i+1900] for i in range(0, len(report), 1900)]
                for i, chunk in enumerate(chunks):
                    await ctx.send(f"全体パフォーマンスレポート (Part {i+1}/{len(chunks)}):\n{chunk}")
            else:
                await ctx.send(report)
        except Exception as e:
            error_msg = f'❌ パフォーマンスレポート取得エラー: {e}'
    ...
    async def command_list(ctx):
        ...
        try:
            ...
            help_msg += "• `performance` - 今日の取引パフォーマンスレポート\n"
            help_msg += "• `performance -1` - 前日の取引パフォーマンスレポート\n"
            help_msg += "• `performance -2` - 2日前の取引パフォーマンスレポート\n"
            help_msg += "• `all` - 全体の取引パフォーマンスレポート\n\n"
...
def collect_metrics(use_today_only=False, days_offset=None):
    ...
    if days_offset is not None:
        target_trades, target_date = get_trades_by_date_offset(days_offset)
    elif use_today_only:
        target_trades = get_today_trades()
    else:
        target_trades = trade_results
    
    # 取引結果から統計を計算
    if target_trades:
        total_profit_pips = sum(trade['profit_pips'] for trade in target_trades)
        total_profit_amount = sum(trade.get('profit_amount', 0) for trade in target_trades)
        successful_trades = len([t for t in target_trades if t['profit_pips'] > 0])
        failed_trades = len([t for t in target_trades if t['profit_pips'] <= 0])
        ...
        performance_metrics.update({
            ...
            'average_profit_pips': total_profit_pips / len(target_trades) if target_trades else 0,
            'win_rate': (successful_trades / len(target_trades) * 100) if target_trades else 0
        })
...
def get_performance_report(use_today_only=False, days_offset=None):
    ...
    if days_offset is not None:
        ...
        if days_offset == -1:
            report = f"**📅 前日のパフォーマンスレポート**\n"
        elif days_offset == -2:
            report = f"**📅 2日前のパフォーマンスレポート**\n"
        else:
            report = f"**📅 {abs(days_offset)}日前のパフォーマンスレポート**\n"
        report += f"対象期間: {target_date.strftime('%Y年%m月%d日')} 0:00 ～ 24:00\n"
    elif use_today_only:
        report = f"**📅 今日のパフォーマンスレポート**\n"
        report += f"対象期間: 今日 0:00 ～ 現在\n"
    else:
        report = f"**📊 全体パフォーマンスレポート**\n"
